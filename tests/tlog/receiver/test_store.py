...
class test_store(Testbase):
    ...
    def test_save(self):
        ...
        store = Store(Parse(u'<34>Oct 11 22:14:15 mymachine.example.com su - ID47 - BOM\'su root\' failed for lonvick on /dev/pts/8 æøå'), [filter_])
        self.assertStore(store, True)

        # Lets test if it works with some unicode letters!
        store = Store(Parse(u'<34>Oct 11 22:14:15 mymachine.example.com su - ID47 - Rød grød med fløde'), [filter_])
        self.assertStore(store, True)
    ...
    def test_count_limit(self):
        store = Store(Parse(u'<34>Oct 11 22:14:15 mymachine.example.com su - ID47 - BOM\'su root\' failed for lonvick on /dev/pts/8 æøå'))
    ...
    def test_get_save_filters(self):
        ...
        store = Store(Parse(u'<34>Oct 11 22:14:15 mymachine.example.com su - ID47 - BOM\'su root\' failed for lonvick on /dev/pts/8 æøå'), [filter1, filter2])
